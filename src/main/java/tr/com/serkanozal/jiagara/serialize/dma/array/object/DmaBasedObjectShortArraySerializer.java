 ...
 * @author Serkan ÖZAL
 */
public class DmaBasedObjectShortArraySerializer<T> extends AbstractDirectMemoryAccessBasedFieldAndDataSerializer<T, DirectMemoryAccessBasedOutputWriter> 
	...
	public DmaBasedObjectShortArraySerializer(Field field) {
		super(field);
		valueFieldOffset = unsafe.objectFieldOffset(ReflectionUtil.getField(Short.class, "value"));
	}
	
	@SuppressWarnings("restriction")
	public DmaBasedObjectShortArraySerializer(Class<T> clazz) {
		super(clazz);
		valueFieldOffset = unsafe.objectFieldOffset(ReflectionUtil.getField(Short.class, "value"));
	}
	...
	public void serializeField(T obj, DirectMemoryAccessBasedOutputWriter outputWriter) {
		writeArray((Short[])unsafe.getObject(obj, fieldOffset), outputWriter);
	}

	@Override
	public void serializeDataContent(T obj, DirectMemoryAccessBasedOutputWriter outputWriter) {
		writeArray((Short[])obj, outputWriter);
	}
	
	protected void writeArray(Short[] array, DirectMemoryAccessBasedOutputWriter outputWriter) {
		if (array == null) {
