 ...
 * @author Serkan ÖZAL
 */
public class DmaBasedPrimitiveLongSerializer<T> extends AbstractDirectMemoryAccessBasedFieldAndDataSerializer<T, DirectMemoryAccessBasedOutputWriter> 
		implements DirectMemoryAccessBasedFieldSerializer<T>, DirectMemoryAccessBasedDataSerializer<T> {
		
	public DmaBasedPrimitiveLongSerializer(Field field) {
		super(field);
	}
	
	public DmaBasedPrimitiveLongSerializer(Class<T> clazz) {
		super(clazz);
	}
	
	@Override
	public void serializeField(T obj, DirectMemoryAccessBasedOutputWriter outputWriter) {
		outputWriter.writeLong(obj, fieldOffset);
	}
