 ...
 * @author Serkan ÖZAL
 */
public class DmaBasedPrimitiveCharacterSerializer<T> extends AbstractDirectMemoryAccessBasedFieldAndDataSerializer<T, DirectMemoryAccessBasedOutputWriter> 
		implements DirectMemoryAccessBasedFieldSerializer<T>, DirectMemoryAccessBasedDataSerializer<T> {
		
	public DmaBasedPrimitiveCharacterSerializer(Field field) {
		super(field);
	}
	
	public DmaBasedPrimitiveCharacterSerializer(Class<T> clazz) {
		super(clazz);
	}
	
	@Override
	public void serializeField(T obj, DirectMemoryAccessBasedOutputWriter outputWriter) {
		outputWriter.writeCharacter(obj, fieldOffset);
	}

	@Override
	public void serializeDataContent(T obj, DirectMemoryAccessBasedOutputWriter outputWriter) {
		outputWriter.write((Character)obj);
	}
