 ...
 * @author Serkan ÖZAL
 */
public class DmaBasedPrimitiveBooleanSerializer<T> extends AbstractDirectMemoryAccessBasedFieldAndDataSerializer<T, DirectMemoryAccessBasedOutputWriter> 
		implements DirectMemoryAccessBasedFieldSerializer<T>, DirectMemoryAccessBasedDataSerializer<T> {
	
	public DmaBasedPrimitiveBooleanSerializer(Field field) {
		super(field);
	}
	
	public DmaBasedPrimitiveBooleanSerializer(Class<T> clazz) {
		super(clazz);
	}

	@Override
	public void serializeField(T obj, DirectMemoryAccessBasedOutputWriter outputWriter) {
		outputWriter.writeBoolean(obj, fieldOffset);
	}
