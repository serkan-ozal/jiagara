 ...
 * @author Serkan ÖZAL
 */
public class DmaBasedPrimitiveDoubleArraySerializer<T> extends AbstractDirectMemoryAccessBasedFieldAndDataSerializer<T, DirectMemoryAccessBasedOutputWriter> 
		implements DirectMemoryAccessBasedFieldSerializer<T>, DirectMemoryAccessBasedDataSerializer<T> {
	
	public DmaBasedPrimitiveDoubleArraySerializer(Field field) {
		super(field);
	}
	
	public DmaBasedPrimitiveDoubleArraySerializer(Class<T> clazz) {
		super(clazz);
	}
	
	@Override
	public void serializeField(T obj, DirectMemoryAccessBasedOutputWriter outputWriter) {
		outputWriter.writeDoubleArray(obj, fieldOffset);
	}
	...
	public void serializeDataContent(T obj, DirectMemoryAccessBasedOutputWriter outputWriter) {
		...
		if (array == null) {
			...
			outputWriter.writeDoubleArray(address);
		}
	}
